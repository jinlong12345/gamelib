import pytest
import os
from games.domainmodel.model import Publisher, Genre, Game, Review, User, Wishlist, make_review, delete_review
...
def test_make_review_method():
    ...
    make_review("Great game!", 3, user, game)
    make_review("Splendid game!", 2, user, game)
    assert len(game.reviews) == 2
    assert len(user.reviews) == 2

def test_delete_review_method():
    user = User("Shyamli", "pw12345")
    game = Game(1, "Domino Game")

    review1 = make_review("Great game!", 3, user, game)
    review2 = make_review("Splendid game!", 2, user, game)
    assert len(game.reviews) == 2
    assert len(user.reviews) == 2

    delete_review(review1)
    assert len(game.reviews) == 1
    assert len(user.reviews) == 1

    delete_review(review2)
    assert len(game.reviews) == 0
    assert len(user.reviews) == 0
